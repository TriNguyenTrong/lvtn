...
class TransformerDecoderLayerMulti(nn.TransformerDecoderLayer):
    ...
    def forward(self, tgt: Tensor, memory1: Tensor, memory2: Tensor, 
                ...
                memory1_key_padding_mask: Optional[Tensor] = None,
                memory2_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        r"""Pass the inputs (and mask) through the decoder layer.

        Args:
            tgt: the sequence to the decoder layer (required).
            memory (1,2): the sequence from the last layer of the encoder (required).
            tgt_mask: the mask for the tgt sequence (optional).
            memory_mask: the mask for the memory sequence (optional).
            tgt_key_padding_mask: the mask for the tgt keys per batch (optional).
            memory(1,2)_key_padding_mask: the mask for the memory keys per batch (optional).
...
class TransformerDecoderMulti(nn.TransformerDecoder):
    def __init__(self, decoder_layer, num_layers, norm=None):
        super().__init__(decoder_layer, num_layers, norm)
    
    def forward(self, tgt: Tensor, memory1: Tensor,  memory2: Tensor, tgt_mask: Optional[Tensor] = None,
        ...
        for mod in self.layers:
            output = mod(output, memory1, memory2, tgt_mask=tgt_mask,
                         memory_mask=memory_mask,
                         tgt_key_padding_mask=tgt_key_padding_mask,
                         memory1_key_padding_mask=memory1_key_padding_mask,
...
class Decoder(pl.LightningModule):
    ...
    def forward(
        self, src1: FloatTensor, src2: FloatTensor, src1_mask: LongTensor, src2_mask: LongTensor, tgt: LongTensor
    ) -> FloatTensor:
        ...
        src1 = rearrange(src1, "b t d -> t b d")
        src2 = rearrange(src2, "b t d -> t b d")
        ...
        out = self.model(
            ...
            memory1_key_padding_mask=src1_mask,
            memory2_key_padding_mask=src2_mask, 
        )
